...
class AppMailer
{
    ...
    public function sendRegisterMail(User $user)
    {
        ...
        <br><br>
        Cliquez sur le lien suivant pour activer votre compte : <a href="http://localhost:8000/confirmAccount&' . $user->getEmail() . '&' . $user->getToken()->getContent() . '">http://localhost:8000/confirmAccount&' . $user->getEmail() . '&' . $user->getToken()->getContent() . '</a>
